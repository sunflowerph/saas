...
#地址、账号、密码信息
test={'url':'http://118.25.66.69:8080/supermarket/login','name':'cccs1','password':'1qazxsw2'} #测试
online={'url':'http://saas.zhuisuyun.net/supermarket/login','name':'ddcs','password':'1qazxsw2'} #线上
hubei={'url':'http://jcyp.hbszyczs.cn/supermarket/login','name':'ddcs','password':'1qazxsw2'} #湖北


#菜单管理
menu_base=['/html/body/div[1]/aside/section/ul/li[9]/a/span[1]','/html/body/div[1]/aside/section/ul/li[9]/ul/li[1]/a']#基础信息管理
menu_staff=['/html/body/div/aside/section/ul/li[9]/a/span[1]','/html/body/div/aside/section/ul/li[9]/ul/li[2]/a']#员工信息管理
...
def add_goods(sku,name,brand,code,durabilityDay,supplier,entname,shelfLifeDay,price):
    ...
    enter_menu(menu_goods)  # 进入产品管理子菜单
...
def edit_base_information(shortname,linkman,phone,law_person,address,picture1,picture2,picture3): #参数：企业简称、联系人、联系方式、法人代表、地址
    enter_menu(menu_base)  # 进入基本信息管理子菜单
...
def add_staff(name,mobile,post,psw): #参数：姓名、联系方式、岗位、密码
    enter_menu(menu_staff)  # 进入员工管理子菜单
...
def stock_registration(supplier,registrant): #参数：供应商、登记人
    enter_menu(menu_stock)  # 进入进货登记子菜单
...
#通过二维码查看批次H5信息
def preview_rq():
    enter_menu(menu_batch)  # 进入批次管理子菜单
    time.sleep(5)
...
def add_testreport(testname,testnumber,tester):
    enter_menu(menu_check)  # 进入检测报告菜单
...
def add_SalesRecord(tester):
    enter_menu(menu_sale)  # 进入销售记录子菜单
...
#预警信息导出（营业执照过期）
def warning_query1():
    enter_menu(menu_warning)  # 进入预警信息子菜单
    time.sleep(3)
